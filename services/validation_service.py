...
def validate_step1(data: dict) -> list:
    ...
    if not re.match(r'^[6-9]\d{9}$', data.get('mobile', '')):
        errors.append('Invalid mobile number')
    if not re.match(r'^[\w\.-]+@[\w\.-]+\.\w+$', data.get('email', '')):
        errors.append('Invalid email address')
    if not re.match(r'^\d{12}$', data.get('aadhaar_number', '')):
        errors.append('Aadhaar must be 12 digits')
    if not re.match(r'^[A-Z]{5}[0-9]{4}[A-Z]$', data.get('pan_number', '').upper()):
...
def validate_step3(data: dict) -> list:
    ...
    if not re.match(r'^[A-Z]{4}0[A-Z0-9]{6}$', data.get('ifsc_code', '').upper()):
        errors.append('Invalid IFSC code format')
    if not re.match(r'^\d{6}$', data.get('pincode', '')):
